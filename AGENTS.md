...
## Current Feature: Region-scoped scheduling

### Context
- Goal: Select a region from local storage, browser geolocation, or a dropdown, then scope schedules, members, history, and Google Calendar to it.
- Key files: `src/contexts/RegionContext.jsx`, `src/pages/`, `supabase/region_scope_setup.sql`.
- Complexity: COMPLEX.

### Architecture
- City coordinates and match radii live in `region_cities`; canonical region names and region/member consistency are enforced in SQL.
- Current regional filtering is application-level because the project has no user-to-region authorization model.

### Validation
- `git diff --check` passes.
- Vitest and production build remain unrun because dependencies are not installed and package retrieval timed out in this environment.
