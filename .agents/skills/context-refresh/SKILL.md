...
description: |
  ...
  Archives older entries from AGENTS.md Zone C, `.Codex/checkpoints/`, and
  `.Codex/docs/research/`, then regenerates a Work Evolution narrative and
  ...
  (typical trigger: AGENTS.md > ~400 lines, checkpoints > 10 files, or
  visible duplication in Session History).
metadata:
  short-description: Compress AGENTS.md Zone C and archive old checkpoints/research
disable-model-invocation: true
---

# Context Refresh

**Maintenance skill that keeps long-running working state slim by archiving older entries and regenerating a compact Work Evolution + Archive Index inside AGENTS.md Zone C.**

This skill is the counterweight to `/checkpointing`, `/start-feature`, `/add-feature`, and `/troubleshoot`, all of which append to Zone C and to `.Codex/checkpoints/` / `.Codex/docs/research/` over time. Without periodic compression, those areas crowd the orchestrator's context budget.
...
- AGENTS.md exceeds ~400 lines or Zone C scrolls off-screen.
- `.Codex/checkpoints/` contains more than ~10 files.
...
2. **Latest 5 checkpoints are sacred**: `/catchup` reads them. They must remain in `.Codex/checkpoints/` (not archived) regardless of age.
...
|---|---|---|
| `.Codex/checkpoints/archive/2026-01.md` | 2026-01 | N sessions, key topics |
| `.Codex/docs/archive/projects-2026-Q1.md` | 2026-Q1 | feat-x, feat-y |
| `.Codex/docs/research/archive/{feature}.md` | ... | ... |
...
- `.Codex/checkpoints/archive/YYYY-MM.md` — month-aggregated checkpoint summaries.
- `.Codex/docs/archive/projects-YYYY-Qn.md` — quarter-aggregated `Current Project|Feature|Bug Fix` blocks.
...
### Checkpoints
- Path: `.Codex/checkpoints/*.md` (NOT including `archive/`)
...
### Existing archives
- Path: `.Codex/checkpoints/archive/`, `.Codex/docs/archive/`,
  `.Codex/docs/research/archive/`
...
### File moves ({count})
- `.Codex/checkpoints/2026-01-12-*.md` (3 files)
    -> merge into `.Codex/checkpoints/archive/2026-01.md`
- `.Codex/docs/research/feat-x.md`
    -> `.Codex/docs/research/archive/feat-x.md`
- `## Current Project: feat-x` (from AGENTS.md)
    -> append to `.Codex/docs/archive/projects-2026-Q1.md`
...
options:
  - label: "proceed"
    description: "Execute the moves and rewrite AGENTS.md Zone C as previewed."
  - label: "adjust"
...
```bash
mkdir -p .Codex/checkpoints/archive
mkdir -p .Codex/docs/archive
mkdir -p .Codex/docs/research/archive
```

### 4.2 Aggregate checkpoint archives (per month)

For each month bucket, write or append to `.Codex/checkpoints/archive/YYYY-MM.md`:
...
Append removed `## Current Project|Feature|Bug Fix` blocks to `.Codex/docs/archive/projects-YYYY-Qn.md`:
...
2. List the contents of `.Codex/checkpoints/`, `.Codex/checkpoints/archive/`, `.Codex/docs/research/`, `.Codex/docs/research/archive/`, `.Codex/docs/archive/`.
3. Re-grep for both boundary markers to confirm they survived intact:
   ...
   Both must return `1`.
4. Provide a single user-facing recap paragraph (Japanese, per `.Codex/rules/language.md`) summarising: lines removed, files archived, latest checkpoints preserved, and where the archives live.
...
| Skill | Relationship |
|---|---|
| `/checkpointing` | Continues to append to Zone C `Session History` and `.Codex/checkpoints/`. `/context-refresh` cleans these up later. No coordination needed. |
...
| `/start-feature`, `/add-feature`, `/troubleshoot` | Continue to append `## Current Project|Feature|Bug Fix` blocks to Zone C. `/context-refresh` retires older blocks into `.Codex/docs/archive/`. |
