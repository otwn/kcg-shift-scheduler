...
description: |
  ...
  Scans repository artifacts (git history, AGENTS.md / AGENTS.md,
...
- .Codex/rules/*.md (every rule file, summarize each in 1–2 lines)

### Capabilities catalog
- .Codex/skills/*/SKILL.md — read only the YAML frontmatter (name,
  description, metadata.short-description). Build a list of available
  slash commands and their one-line purposes.
- .Codex/agents/*.md — list subagent types and their specializations
  (frontmatter only).

### Design & research context
- .Codex/docs/DESIGN.md (architecture decisions, key choices)
- .Codex/docs/research/*.md (each research note, 1–2 line summary)
- .Codex/docs/libraries/*.md (library constraints, 1–2 line summary)

### Local history (scan ONLY if the path exists)
- .Codex/checkpoints/*.md (newest 5, summarize each in 1–2 lines)
- .Codex/logs/agent-teams/*/*.md (teammate work logs, group by team)
- .Codex/logs/cli-tools.jsonl (last ~50 entries; extract distinct
...
### Current state
- Active branch + any "Current Project" section from AGENTS.md
...
- Active research threads (from .Codex/docs/research/)
- Library constraints worth knowing (from .Codex/docs/libraries/)
...
### Rules summary
- One-line summary per .Codex/rules/*.md file
...
{List from .Codex/docs/research/ with 1-line summaries}

### Library constraints
{List from .Codex/docs/libraries/ with 1-line summaries}
...
_Sources: git log, AGENTS.md, AGENTS.md, .Codex/rules/, .Codex/skills/,
.Codex/docs/, .Codex/checkpoints/, .Codex/logs/._
```
...
- **`.gitignore` awareness**: `.Codex/checkpoints/` and `.Codex/logs/` are gitignored. On a fresh clone they will be absent; the skill handles this gracefully.
